...
import gov.va.ehtac.appsonfhir.dataresults.ImmunizationResults;
...
public class Immunizations extends NavigationView {
    ...
    public Immunizations() {
        ...
        setCaption("Patient Immunizations - "+session.getPatientNameAgeGenderDisplay());
...
//        immunizationTable.setHeight("250px");
        ...
        immunizationTable.setContainerDataSource(populatePatientConditions());

        immunizationTable.setColumnReorderingAllowed(true);
        immunizationTable.setColumnCollapsingAllowed(false);
        immunizationTable.setVisibleColumns(new Object[] {"selected",  "displayName", "adminDate", "refused", "refusalReason", "reported"});
...
//    }
    ...
    private String getImmunizationSearchString() {
        String res = session.getBaseURL()+"Immunization?subject=Patient/patient-"+session.getPatientId();
