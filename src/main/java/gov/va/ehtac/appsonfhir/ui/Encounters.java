...
//        encounterTable.setHeight("250px");
        ...
        encounterTable.setContainerDataSource(populatePatientConditions());

        encounterTable.setColumnReorderingAllowed(true);
        encounterTable.setColumnCollapsingAllowed(false);
        encounterTable.setVisibleColumns(new Object[] {"selected", "startdate", "enddate", "encountertype", "status", "displayname"});
        encounterTable.setColumnHeaders(new String[] {"", "Start Date", "End Date", "Type", "Status", "Name"});
...
//    }
    ...
    private String getEncounterSearchString() {
        String res = session.getBaseURL()+"Encounter?patient=Patient/patient-"+session.getPatientId();
